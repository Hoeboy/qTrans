...
namespace SurveyMath {
	...
	private:
		...
		double sm_b = 6356752.31424518;
		double sm_EccSquared = 6.69437999013e-03;
		double UTMScaleFactor = 0.9996; 

		typedef BasePoint UTMCoor;

		typedef BasePoint WGS84Corr;
		/*
		* DegToRad
		*
		...
		*/
		void MapXYToLatLon(double x, double y, double lambda0, WGS84Corr &philambda)
		{
			...
			/* Calculate latitude */
			philambda.x = phif + x2frac * x2poly * (x * x) + x4frac * x4poly * pow(x, 4.0) + x6frac * x6poly * pow(x, 6.0) + x8frac * x8poly * pow(x, 8.0);

			/* Calculate longitude */
			philambda.y = lambda0 + x1frac * x + x3frac * x3poly * pow(x, 3.0) + x5frac * x5poly * pow(x, 5.0) + x7frac * x7poly * pow(x, 7.0);
		}
